...
if sys.argv[-1] == 'dry-run':
    ...
    if branch != "master":
        sys.exit("dry-run from a branch other than master is disallowed.")
    os.system("pip uninstall -y hcli_hai")
    os.system("rm -rf requirements.txt")
    write_requirements()
    os.system("rm -rf hcli_hai.egg-info")
...
if sys.argv[-1] == 'publish':
    ...
    os.system("pip install -e .")
    os.system("git tag -a %s -m 'version %s'" % ("hcli_hai-" + version, "hcli_hai-" + version))
    os.system("git push")
